...
main (void)
{
  ...
  argv = parse_options (argv); /* now, argv only have non-option part */ 

  /* Initialize ourselves as a thread so we can use locks,
     then enable console locking. */
  thread_init (); /* 1 lock, 2 lists init here */
  console_init ();
  ...
  intr_init ();/* write sth to HW; init intr_names[256](strings) */
  timer_init ();
  kbd_init ();
  input_init ();
#ifdef USERPROG
  ...
  run_actions (argv);/* Seems all these runing context is controlled by intr */ 

  /* Finish up. */ 
  shutdown (); /* Don't change its order! */
  thread_exit ();
}

...
read_command_line (void) 
{
  ...
  argc = *(uint32_t *) ptov (LOADER_ARG_CNT);/* ptov: original + base */ 
  p = ptov (LOADER_ARGS);/* these very beginning addr is almost fixed */ 
  ...
  for (i = 0; i < argc; i++) 
    {
      ...
      argv[i] = p;/* we get the command from there?? */
      p += strnlen (p, end - p) + 1;/* very... magical */
    }
  argv[argc] = NULL;
  ...
  for (i = 0; i < argc; i++)
    if (strchr (argv[i], ' ') == NULL)
      printf (" %s", argv[i]);/* maybe I should learn... */
    else
      printf (" '%s'", argv[i]);
  printf ("\n");
...
run_actions (char **argv) 
{
  ...
  static const struct action actions[] = 
    {
      {"run", 2, run_task},/* run_task(char **argv) */
#ifdef FILESYS
      {"ls", 1, fsutil_ls},
      {"cat", 2, fsutil_cat},
      {"rm", 2, fsutil_rm},
  ...
  while (*argv != NULL)
    {
      ...
      /* Find action name. */
      for (a = actions; ; a++)
        if (a->name == NULL)/* the use of {NULL, 0, NULL} */
          PANIC ("unknown action `%s' (use -h for help)", *argv);
        else if (!strcmp (*argv, a->name))
          break;
      
      /* Check for required arguments. */ /* a-> one of the action (in actions[]) */
      for (i = 1; i < a->argc; i++)
        if (argv[i] == NULL)
          PANIC ("action `%s' requires %d argument(s)", *argv, a->argc - 1);

      /* Invoke action and advance. */
      a->function (argv);/* for P1, goto run_task */
      argv += a->argc;/* we can have many <run(1) action(1)>s */
    }
