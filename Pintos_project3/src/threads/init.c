...
static void
paging_init (void) //#A kernel-pd for all 4MB
{
  uint32_t *pd, *pt;
  size_t page;
      ...
      if (pd[pde_idx] == 0)
        {
          ...
          pd[pde_idx] = pde_create (pt); //#A PTE_U | PTE_P | PTE_W ok for pt
        }
      //#A kernel no-page-fault in kernel-vm
      pt[pte_idx] = pte_create_kernel (vaddr, !in_kernel_text); //#A -3GB, P=true for all
    }
