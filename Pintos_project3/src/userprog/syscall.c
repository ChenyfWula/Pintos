...
bool is_valid_uptr(void *uptr)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    //@3-2 in: is_valid_uptr
    if (get_user (uptr + i) == -1)
      return false;
  }
  return true;
}
...
static void sc_read(struct intr_frame *f)
{
  ...
  //@3-2 in: sc_read
  if(find_page_info(buffer, thread_current())->writable == false){
    f->eax = -1;
    error_exit();
  }
  //==========validation finish=============
...
//@3-4 F: sc_mmap: mapid_t mmap (int fd, void *addr)
static void sc_mmap(struct intr_frame *f){
  ...
  f->eax = file_map_create(fd, addr); //#A a/r lock inside
  return;
}
//@3-4 F: sc_munmap: void munmap (mapid_t)
...
void syscall_init(void)
{
  ...
  sys_func_table[13] = sc_mmap;
  sys_func_table[14] = sc_munmap;
